...
adapter.findAll = function(query, callback) {
  ...
  async.series([
    ...
    function(next) {
      ...
      var sql = Model.adapter.buildSQL(idQuery);
    ...
    function(next) {
      ...
      if (allQuery.include) {
        ...
        if (typeof allQuery.include == 'string') {
          allQuery.include = allQuery.include.split(',');
        }
        allQuery.where = { id: { $in: ids } };
      }
      var sql = Model.adapter.buildSQL(allQuery);
      Model.adapter.query(sql.query, sql.values, function(err, rows) {
        ...
        Model.relations.forEach(function(relation) {
          if (!~query.include.indexOf(relation.as) || relation.model != Model) {
            return;
          }

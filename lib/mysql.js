...
adapter.findAll = function(query, callback) {
  ...
  var ids = [];
  var collection = [];
  collection.limit = Number(query.limit || 50);
  collection.offset = Number(query.offset || 0);
  collection.total = 0;
      ...
      Model.adapter.query(sql.query, sql.values, function(err, rows) {
        ...
        if (sql.relations) {
          ...
          for (var len = collection.length, i=0; i<len; i++) {
            if (!ids[collection[i][Model.primaryKey]]) {
              ids[collection[i][Model.primaryKey]] = 1;
          ...
          for (var plural in sql.relations) {
            ...
            for (var l = collection.length, ii=0; ii<l; ii++) {
              var model = collection[ii];
              ...
              for (var len = rows.length, i=0; i<len; i++) {
                ...
                if (foreignkey == model[Model.primaryKey]) {
                  var related = transformColumnNames(stripTableName(rows[i], relation.anotherModel.options.tableName), relation.anotherModel);
    ...
    if (sql.relations) {
      ...
      for (var plural in sql.relations) {
        ...
        for (var len = rows.length, i=0; i<len; i++) {
          ...
          var related = transformColumnNames(
            stripTableName(rows[i], relatedModel.options.tableName),
            relatedModel
          );
          model.related[plural].push(related);
        }
  ...
  var after = function(rows, fields) {
    if (rows && rows.length) {
      rows.forEach(function(row, i) {
        ...
        rows[i] = transformColumnNames(
          stripTableName(row, Model.options.tableName),
          Model
        );
      });
    }
...
function transformColumnNames(row, Model) {
  var converted = {};
  columnsToAttributes = {};
  ...
  for (var key in row) {
    if (columnsToAttributes[key]) {
      converted[columnsToAttributes[key]] = row[key];
      delete row[key];
    }
    else {

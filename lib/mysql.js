  ...
  async.series([
    ...
    function(next) {
      ...
      Model.adapter.query(sql.query, sql.values, function(err, rows) {
        ...
        if (!rows || !rows.length) return next();
        for (var len = rows.length, i=0; i<len; i++) {
          collection.push(stripTableName(rows[i], Model.options.tableName));
        }
        if (sql.relations) {
  ...
  var after = function(rows, fields) {
    if (rows.length) {
      ...
      if (columnNames.length) {
        for (var l = rows.length, i=0; i<l; i++) {
          for (var ll = columnNames.length, ii=0; ii<ll; ii++) {
            var tableColumnName = Model.options.tableName + '_' + columnNames[ii];
            if (rows[i][tableColumnName]) {
              rows[i][Model.options.tableName + '_' + columnNamesToAttrNames[columnNames[ii]]] = rows[i][tableColumnName];
...
function prepareQuery(Model, query) {
  ...
  for (var param in query) {
    ...
    if (!param.match(/(updates|join|include|columns|table|type|values|where|offset|limit|sort|order|groupBy)$/i)) {
      where = where || {};

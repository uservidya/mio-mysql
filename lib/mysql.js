        ...
        if (sql.relations) {
          ...
          for (var plural in sql.relations) {
            ...
            for (var l = results.length, ii=0; ii<l; ii++) {
              ...
              for (var len = rows.length, i=0; i<len; i++) {
                var foreignkey = rows[i][(relation.through || relation.model).options.tableName + '_foreign_key'];
  ...
  if (query.include) {
    ...
    include.forEach(function(relation) {
      ...
      if (relation.through) {
        ...
        query.leftOuterJoin[relation.anotherModel.tableName] = {};
      ...
      query.columns.push({ name: relation.foreignKey, table: (relation.through || relation.model).options.tableName, as: 'foreign_key' });

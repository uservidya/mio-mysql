        ...
        Model.relations.forEach(function(relation) {
          ...
          for (var len = rows.length, i=0; i<len; i++) {
            ...
            if (related_id) {
              ...
              transformRowColumns(rows[i], relation.anotherModel, true);
              var related = stripTableName(rows[i], relation.anotherModel);
        ...
        if (~query.include.indexOf(relation.as) && relation.model == this) {
          ...
          for (var len = rows.length, i=0; i<len; i++) {
            ...
            if (rows[i][foreignKey]) {
              ...
              transformRowColumns(rows[i], anotherModel, true);
              var related = stripTableName(rows[i], anotherModel);
  ...
  if (query.include && query.type != 'count') {
    ...
    query.include.forEach(function(name) {
      ...
      if (relation.through) {
        ...
        joinThrough.on[relation.anotherModel.primaryKey] = '$' + relation.through.options.tableName + '.' + relation.throughKey + '$';

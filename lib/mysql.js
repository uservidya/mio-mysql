 ...
 * connection, use `Model.adapter.settings.pool`.
 */
...
adapter.save = function(changed, done) {
  if (this.primary && !changed[this.constructor.primaryKey]) {
    return adapter.update.call(this, changed, done);
  }
...
adapter.query = function(statement, values, callback) {
  var Model = this;
  var pool = Model.adapter.settings.pool;
  ...
  pool.getConnection(function(err, conn) {
    ...
    function done(err, rows, fields) {
      ...
      conn.canceled ? conn.destroy() : conn.release();
    ...
    }, Model.adapter.settings.queryTimeout);

    conn.query(statement, values, function(err, rows, fields) {
...
adapter.queryToSQL = function(query) {
  ...
  if (query.where) {
    for (var key in query.where) {
      Model.relations.forEach(function(params) {
        if (params.anotherModel === Model) {
          if (params.foreignKey == key) {
            relation = params;
          }
          else if (params.type == 'has one' && params.model.options.tableName + '_' + params.model.primaryKey == key) {
            relation = params;
          }
        }
      });
  ...
  if (relation && (relation.through || relation.type == 'has one')) {
    var join = { type: 'inner', on: {} };
    if (relation.through) {
      ...
      join.target = relation.model.options.tableName;
      join.on[relation.foreignKey] = '$' + relation.anotherModel.options.tableName + '.' + relation.anotherModel.primaryKey + '$';
      delete query.where[relation.model.options.tableName + '_' + relation.model.primaryKey];
    }
    ...
    query.include.forEach(function(name) {
      ...
      if (relation.through) {
        ...
        if (relation.type == 'has one') {
          joinRelated.on[relation.anotherModel.primaryKey] = '$' + Model.options.tableName + '.' + relation.foreignKey + '$';
        }
        else {
      ...
      if (relation.type == 'has one') {
        query.columns.push({ name: relation.anotherModel.primaryKey, table: (relation.through || relation.anotherModel).options.tableName, as: 'foreign_key' });
      }
      else {
...
function queryFormat(query, values) {
  ...
  var statement = query.replace(/\$\d+/g, function(match) {
